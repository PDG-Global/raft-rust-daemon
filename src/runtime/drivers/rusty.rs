//! RustyCLI runtime driver.
//!
//! This driver implements the Runtime trait using RustyCLI (rustycli.com).
...
/// RustyCLI runtime configuration.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct RustyRuntimeConfig {
    /// The RustyCLI binary path.
...
pub struct RustyRuntime {
    /// The runtime config.
    config: RustyRuntimeConfig,
    /// The RustyCLI process.
...
impl RustyRuntime {
    /// Create a new RustyCLI runtime.
    pub async fn new(config: RustyRuntimeConfig) -> Result<Self> {
        // Check if binary exists
        if !config.binary_path.exists() {
            return Err(anyhow::anyhow!(
                "RustyCLI binary not found at: {}",
        ...
        let process = TokioCommand::new(&config.binary_path)
            ...
            .stderr(Stdio::piped())
            .spawn()
            .context("Failed to start RustyCLI process")?;
    ...
    pub async fn handle_message(&mut self, message: Message) -> Result<AgentResponse> {
        ...
        let process = self
            .process
            .as_mut()
            .ok_or_else(|| anyhow::anyhow!("RustyCLI process not running"))?;
...
impl Runtime for RustyRuntime {
    ...
    async fn handle_message(&self, message: Message) -> Result<AgentResponse> {
        ...
        let runtime = RustyRuntime::new(self.config.clone()).await?;
